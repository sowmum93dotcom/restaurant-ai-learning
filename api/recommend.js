...
const proposalPattern = /\b(?:create|created|creating|develop|developed|developing|introduce|introduced|introducing|propose|proposed|proposing|consider|considered|considering|test|tested|testing|explore|explored|exploring|design|designed|designing)\b/i;
...
function containsUnsupportedBusinessPremise(text, suppliedText) {
  ...
  return unsupportedAssetPatterns.some(({ re }) => {
    ...
    return matches.some((match) => {
      ...
      const nearby = text.slice(Math.max(0, index - 45), Math.min(text.length, index + value.length + 45));
      return !proposalPattern.test(nearby);
    });
  });
}
