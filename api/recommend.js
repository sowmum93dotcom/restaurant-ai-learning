...
const proposalPattern = /\b(?:create|creating|develop|developing|introduce|introducing|propose|proposing|consider|considering|test|testing|explore|exploring|design|designing)\b/i;
...
function containsUnsupportedBusinessPremise(text, suppliedText) {
  ...
  return unsupportedAssetPatterns.some(({ re }) => {
    const matches = text.match(new RegExp(re.source, "ig"));
    if (!matches) return false;
    return matches.some((match) => {
      if (suppliedText.includes(match.toLocaleLowerCase())) return false;
      const index = text.toLocaleLowerCase().indexOf(match.toLocaleLowerCase());
      const nearby = text.slice(Math.max(0, index - 45), Math.min(text.length, index + match.length + 45));
