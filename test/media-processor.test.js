...
test("image processor inspects actual media before optimization",async()=>{
 const calls=[];const p=createMediaProcessor({
  ...
  optimizeImage:async()=>{calls.push("optimize");return{deliveryUrl:"https://cdn.example/i.webp",derivatives:[{role:"customer",deliveryUrl:"https://cdn.example/c.webp",width:1200,height:900}]};}
 });const r=await p(image,{jobId:"1"});assert.equal(r.success,true);assert.deepEqual(calls,["inspect","optimize"]);assert.equal(r.width,1600);
});
...
test("video processor returns verified metadata and optimized derivatives",async()=>{
 const p=createMediaProcessor({
  ...
  transcodeVideo:async()=>({deliveryUrl:"https://cdn.example/v.mp4",derivatives:[{role:"customer",deliveryUrl:"https://cdn.example/vc.mp4",contentType:"video/mp4",width:720,height:1280,durationSeconds:24}]})
 });const r=await p(video);assert.equal(r.success,true);assert.equal(r.durationSeconds,24);assert.equal(r.derivatives.length,1);
});
