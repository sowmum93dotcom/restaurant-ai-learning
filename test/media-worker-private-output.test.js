...
test("worker accepts processor result only after every delegated output is verified",async()=>{
 const asset={assetId:"a1",businessId:"b1",kind:"image",state:"processing",storageKey:"businesses/b1/media/a1/original",contentType:"image/png"};let saved=false,completed=false;
 const repository={async recoverStaleMediaProcessingJobs(){return[];},async reconcileUnqueuedProcessingMedia(){return[];},async claimNextMediaProcessingJob(){return{job_id:1,asset_id:"a1",business_id:"b1",attempts:1};},async getBusinessMediaAsset(){return asset;},async saveBusinessMediaAsset(_businessId,ready){saved=true;return ready;},async finishMediaProcessingJob(_jobId,success){completed=success;return{status:"completed"};}};
 const worker=createMediaWorker({repository,
  drivers:{async inspectImage(){return{contentType:"image/png",width:1200,height:800};},async optimizeImage(_a,_i,ctx){const m=ctx.outputDestinations.find(x=>x.role==="master"),d=ctx.outputDestinations.filter(x=>x.role!=="master");return{storageKey:m.storageKey,deliveryUrl:"https://delivery.example/master.webp",derivatives:d.map(x=>({role:x.role,storageKey:x.storageKey,deliveryUrl:"https://delivery.example/"+x.role+".webp",width:1200,height:800}))};}},
  async createProcessingRead({storageKey,expiresAt}){return{storageKey,readUrl:"https://private.example/read",expiresAt};},
  async createProcessingWrite(o){return{...o,uploadUrl:"https://private.example/write"};},
  async verifyProcessedOutput({storageKey}){return{exists:true,storageKey,contentType:"image/webp"};}
 });
 const result=await worker();assert.equal(result.status,"completed");assert.equal(saved,true);assert.equal(completed,true);
});
